...
/* ---------- 物品使用预览 itemUsePreview（修复「炼出来却不能服用」）---------- */
{
  ...
  const wHigh = S.itemUsePreview(st0, { 名称: '护身符', 类型: '消耗品', effect: { ward: true } });
  ok(wHigh.mode === 'auto' && wHigh.text.includes('挡去重伤与修为倒退') && wHigh.text.includes('护住灵石'), '高阶护身符预览按实际口径说明挡重伤+护灵石');
  const wLow = S.itemUsePreview(st0, { 名称: '兽皮护符', 类型: '消耗品', effect: { ward: true } });
  ok(wLow.mode === 'auto' && wLow.text.includes('护住灵石') && wLow.text.includes('重伤仍会承受'), '低阶护符预览不虚报挡重伤（消除假承诺）');
